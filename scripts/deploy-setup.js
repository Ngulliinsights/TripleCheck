#!/usr/bin/env tsx
"use strict";
/**
 * Deployment setup script
 * Prepares the application for deployment by checking dependencies,
 * running tests, and building the application
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var child_process_1 = require("child_process");
var cleanup_redundancies_1 = require("./cleanup-redundancies");
var REQUIRED_ENV_VARS = [
    'DATABASE_URL',
    'JWT_SECRET',
];
var OPTIONAL_ENV_VARS = [
    'GOOGLE_API_KEY',
    'SMTP_HOST',
    'SMTP_USER',
    'SMTP_PASS',
];
function checkEnvironmentVariables() {
    console.log('🔍 Checking environment variables...');
    var missing = [];
    var warnings = [];
    // Check required variables
    REQUIRED_ENV_VARS.forEach(function (varName) {
        if (!process.env[varName]) {
            missing.push(varName);
        }
    });
    // Check optional but recommended variables
    OPTIONAL_ENV_VARS.forEach(function (varName) {
        if (!process.env[varName]) {
            warnings.push(varName);
        }
    });
    if (missing.length > 0) {
        console.error('❌ Missing required environment variables:');
        missing.forEach(function (varName) { return console.error("  - ".concat(varName)); });
        process.exit(1);
    }
    if (warnings.length > 0) {
        console.warn('⚠️  Missing optional environment variables:');
        warnings.forEach(function (varName) { return console.warn("  - ".concat(varName)); });
    }
    console.log('✅ Environment variables check passed');
}
function runTests() {
    console.log('🧪 Running tests...');
    try {
        (0, child_process_1.execSync)('npm test -- --run', { stdio: 'inherit' });
        console.log('✅ Tests passed');
    }
    catch (error) {
        console.error('❌ Tests failed');
        process.exit(1);
    }
}
function buildApplication() {
    console.log('🏗️  Building application...');
    try {
        (0, child_process_1.execSync)('npm run build', { stdio: 'inherit' });
        console.log('✅ Build completed');
    }
    catch (error) {
        console.error('❌ Build failed');
        process.exit(1);
    }
}
function checkBuildOutput() {
    console.log('📦 Checking build output...');
    var requiredFiles = [
        'dist/index.html',
        'dist/assets',
    ];
    var missing = requiredFiles.filter(function (file) { return !cleanup_redundancies_1.default.existsSync(file); });
    if (missing.length > 0) {
        console.error('❌ Missing build files:');
        missing.forEach(function (file) { return console.error("  - ".concat(file)); });
        process.exit(1);
    }
    console.log('✅ Build output verified');
}
function generateDeploymentInfo() {
    var deploymentInfo = {
        timestamp: new Date().toISOString(),
        nodeVersion: process.version,
        npmVersion: (0, child_process_1.execSync)('npm --version', { encoding: 'utf8' }).trim(),
        gitCommit: (0, child_process_1.execSync)('git rev-parse HEAD', { encoding: 'utf8' }).trim(),
        gitBranch: (0, child_process_1.execSync)('git rev-parse --abbrev-ref HEAD', { encoding: 'utf8' }).trim(),
        environment: process.env.NODE_ENV || 'development',
    };
    cleanup_redundancies_1.default.writeFileSync('dist/deployment-info.json', JSON.stringify(deploymentInfo, null, 2));
    console.log('📋 Deployment info generated');
}
function main() {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            console.log('🚀 Starting deployment setup...\n');
            try {
                checkEnvironmentVariables();
                runTests();
                buildApplication();
                checkBuildOutput();
                generateDeploymentInfo();
                console.log('\n✅ Deployment setup completed successfully!');
                console.log('🎉 Ready to deploy to production');
            }
            catch (error) {
                console.error('\n❌ Deployment setup failed:', error);
                process.exit(1);
            }
            return [2 /*return*/];
        });
    });
}
if (import.meta.url === "file://".concat(process.argv[1])) {
    main();
}
