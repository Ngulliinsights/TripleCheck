...
// Missing module: ../config/vitest/chunk-generator
// import { generateChunkConfigs, createChunkConfig } from '../config/vitest/chunk-generator'

// Mocks for missing module
const generateChunkConfigs = async (): Promise<ChunkConfig[]> => {
  console.warn('⚠️ generateChunkConfigs is mocked because the module is missing');
  return [];
};
const createChunkConfig = (chunk: ChunkConfig): any => {
  return {};
};
...
async function generateChunkFiles(): Promise<void> {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
${chunk.testFiles.map((file: string) => `      "${file}",`).join('\n')}
    ],
...
`
    ...
    const summary: ChunkSummary = {
      ...
      totalTests: chunks.reduce((sum: number, chunk: ChunkConfig) => sum + chunk.testFiles.length, 0),
      priorityBreakdown: {
        high: chunks.filter((c: ChunkConfig) => c.priority === 'high').length,
        medium: chunks.filter((c: ChunkConfig) => c.priority === 'medium').length,
        low: chunks.filter((c: ChunkConfig) => c.priority === 'low').length,
      }
    }
