...
function useEnhancedImageGallery(
...
function usePropertyActions(
  ...
  const handleVerify = useCallback(
    (event: React.MouseEvent) => {
      handleAction(event, () => callbacks.onVerify?.(property.id), "Verify");
    },
    [handleAction, callbacks, property.id]
  );
...
const LandImageSection = memo(
  ({
    ...
    const statusConfig =
      VERIFICATION_STATUS_CONFIG[property.verificationStatus];
        ...
        <div className="absolute top-2 right-2 z-10 space-y-1">
          ...
          {(() => {
            ...
            return (
              <Badge
                className={cn(
                  "bg-white/95 backdrop-blur-md shadow-sm border-0 text-xs",
                  riskConfig.color
                )}
              >
...
export const EnhancedLandCard = memo<EnhancedLandCardProps>(
  ({
    ...
    const { addToCompare, removeFromCompare, isSelected, canAddMore } =
      useCompare();
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-2 gap-2 sm:gap-0">
              ...
              {property.lastVerified && (
                <div className="text-xs text-muted-foreground text-center sm:text-right">
                  Last verified:{" "}
                  {new Date(property.lastVerified).toLocaleDateString()}
                </div>
              )}
            </div>
