...
interface TrustMetric {
  ...
  readonly category: 'verification' | 'coverage' | 'performance' | 'trust';
}
...
interface ClientLogo {
  ...
  readonly category: 'bank' | 'government' | 'developer' | 'agency';
  readonly country: string;
}

interface TrustIndicatorsProps {
  readonly variant?: 'compact' | 'detailed' | 'hero';
  readonly showClientLogos?: boolean;
  readonly showLiveStats?: boolean;
...
export function TrustIndicators({
  ...
  const filteredMetrics = useMemo(() => {
    if (selectedCategory === 'all') return TRUST_METRICS;
    return TRUST_METRICS.filter(metric => metric.category === selectedCategory);
  }, [selectedCategory]);

  // Categories for filtering
  const categories = useMemo(() => [
    { id: 'all', label: 'All Metrics', count: TRUST_METRICS.length },
    { id: 'verification', label: 'Verification', count: TRUST_METRICS.filter(m => m.category === 'verification').length },
    { id: 'coverage', label: 'Coverage', count: TRUST_METRICS.filter(m => m.category === 'coverage').length },
    { id: 'performance', label: 'Performance', count: TRUST_METRICS.filter(m => m.category === 'performance').length },
    { id: 'trust', label: 'Trust & Safety', count: TRUST_METRICS.filter(m => m.category === 'trust').length },
  ], []);
  ...
  useEffect(() => {
    ...
    const animateValue = (metric: TrustMetric) => {
  ...
  const formatValue = useCallback((metric: TrustMetric): string => {
    const animatedValue = animatedValues[metric.id];
    if (animatedValue === undefined) return metric.value;
    ...
    if (originalValue.includes('%')) {
      return `${(animatedValue / 1000).toFixed(1)}%`;
    }
  ...
  if (variant === 'compact') {
    return (
      <div className={`py-12 ${className}`} id="trust-indicators">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
          {TRUST_METRICS.slice(0, 4).map((metric) => (
            <div key={metric.id} className="text-center">
              <div className={`inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-100 mb-3 ${metric.color}`}>
                {metric.icon}
              </div>
  ...
  if (variant === 'hero') {
    return (
      <div className={`py-16 ${className}`} id="trust-indicators">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-5xl mx-auto">
          {TRUST_METRICS.slice(0, 4).map((metric, index) => (
            <div 
              key={metric.id} 
              className="text-center"
              style={{ animationDelay: `${index * 150}ms` }}
            >
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full bg-white/25 mb-4 shadow-lg ${metric.color}`}>
                {metric.icon}
              </div>
              <div className={`text-3xl font-bold mb-2 text-shadow-sm ${metric.color}`}>
                {showLiveStats ? formatValue(metric) : metric.value}
              </div>
  ...
  return (
    <div className={`py-20 bg-gradient-to-br from-gray-50 to-gray-100/50 ${className}`} id="trust-indicators">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
          {filteredMetrics.map((metric, index) => (
            <Card 
              key={metric.id} 
              className="h-full hover:shadow-lg transition-all duration-300 hover:scale-105 group border-0 shadow-sm"
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <CardContent className="p-6 text-center">
                <div className={`inline-flex items-center justify-center w-14 h-14 rounded-full bg-gray-100 mb-4 transition-all duration-300 group-hover:scale-110 ${metric.color}`}>
                  {metric.icon}
                </div>
          ...
          <div className="text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 items-center opacity-70 hover:opacity-100 transition-opacity duration-300">
              {CLIENT_LOGOS.map((client) => (
                <div 
                  key={client.id}
                  className="flex items-center justify-center p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-200 group"
                >
                  <div className="text-center">
