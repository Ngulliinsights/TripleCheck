...
const hasSessionId = (image: PropertyImage): image is ExtendedPropertyImage => {
  return 'sessionId' in image && typeof (image as ExtendedPropertyImage).sessionId === 'string';
};

const hasError = (image: PropertyImage): image is ExtendedPropertyImage => {
  return 'error' in image && typeof (image as ExtendedPropertyImage).error === 'object';
};

const hasDocumentAuthResult = (image: PropertyImage): image is ExtendedPropertyImage => {
  return 'documentAuthResult' in image && typeof (image as ExtendedPropertyImage).documentAuthResult === 'object';
};
...
const createOptimizedMockServices = () => {
  const storageService = {
    ...
    updateImageMetadata: async (_imageId: string, _metadata: Partial<PropertyImage>) => { 
  ...
  const documentAuthServiceForWorkflow = {
    authenticateDocument: async (fileReference: string, documentType: DocumentType): Promise<any> => {
      await new Promise(resolve => setTimeout(resolve, 200));
      const randomValue = secureRandom();
      const isAuthentic = randomValue > 0.2; // 80% chance of being authentic
  ...
  const auditService = {
    ...
    logValidationEvent: async (_event: string, _metadata: PropertyImageMetadata) => { 
      // Mock implementation - no-op for demo 
    },
    logWorkflowEvent: async (_event: string, _metadata: PropertyImageMetadata) => { 
      // Mock implementation - no-op for demo 
    },
  };
...
const workflowManager = new PropertyImageWorkflowManager({
  ...
  metadataService: {
    ...
    performVirusScan: async (_fileRef: string) => { 
      await new Promise(resolve => setTimeout(resolve, 100));
      const isClean = secureRandom() > 0.05;
      return { clean: isClean, threats: isClean ? [] : ['EICAR-Test-File'] }; // 95% clean
    },
    checkCompliance: async (_fileRef: string, _metadata: PropertyImageMetadata) => {
      await new Promise(resolve => setTimeout(resolve, 75));
      const complianceFlags = secureRandom() > 0.8 ? ['missing_title_deed'] : [];
      const regulatoryFlags = secureRandom() > 0.9 ? ['nema_clearance_required'] : [];
...
// Optimized ProgressBar component - removed inline styles completely
    ...
    <div className="mt-3">
      ...
      <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1">
        <div 
          ...
          // Using CSS custom property instead of inline style for better separation of concerns
          style={{ '--progress-width': `${clampedProgress}%`, width: `var(--progress-width)` } as React.CSSProperties}
        />
      </div>
    </div>
  );
...
const PropertyImageVault: React.FC<PropertyImageVaultProps> = ({
  ...
  }, [landVerificationId, defaultDocumentType, maxConcurrentUploads, enableAuditLogging, onUploadComplete, onUploadError, onProgressUpdate, onWorkflowUpdate]);
  ...
  const [currentDocumentType, setCurrentDocumentType] = useState<DocumentType>(defaultDocumentType);
  ...
  const handleUploadClick = useCallback(async () => {
    if (selectedFiles && selectedFiles.length > 0) {
      try {
        ...
        console.warn('Upload failed:', { error: errorMessage, fileCount: selectedFiles.length });
  ...
  const documentTypeOptions = useMemo(() => {
    ...
    return [
      <option key="property_photo" value="property_photo">Property Photo</option>,
      <option key="title_deed" value="title_deed">Title Deed</option>,
      <option key="survey_plan" value="survey_plan">Survey Plan</option>,
      <option key="valuation_report" value="valuation_report">Valuation Report</option>,
      <option key="identification_document" value="identification_document">Identification Document</option>,
      <option key="other_document" value="other_document">Other Document</option>,
    ];
  }, [allowedDocumentTypes]);

  const renderImageCard = useCallback((image: PropertyImage) => {
    const extendedImage = image as ExtendedPropertyImage;
    const workflowStatus = workflowStats.activeWorkflows > 0 ? workflowManager.getWorkflowStatus(image.id) : null;
    const currentStep = workflowStatus?.currentStep || 'N/A';
    const progress = workflowStatus?.progress || image.progress || 0;
    const statusColorClass = STATUS_COLORS[image.status] || 'bg-gray-200 text-gray-800';
    const approvalColorClass = APPROVAL_STATUS_COLORS[image.approvalStatus] || 'bg-gray-200 text-gray-800';
    const fraudRisk = image.fraudDetectionScore ? formatRiskScore(image.fraudDetectionScore) : null;

    return (
      <div key={image.id} className="border rounded-lg shadow-sm p-4 mb-4 bg-white flex flex-col md:flex-row items-start space-x-4">
        ...
        <div className="flex-grow">
          <h3 className="text-lg font-semibold text-gray-900 truncate" title={image.file.name}>{image.file.name}</h3>
          <p className="text-sm text-gray-600">{formatFileSize(image.file.size)}</p>
          
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColorClass}`}>
              {image.status.charAt(0).toUpperCase() + image.status.slice(1)}
            </span>
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${approvalColorClass}`}>
              Approval: {formatApprovalStatus(image.approvalStatus)}
            </span>
            {image.documentType && (
              <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 flex items-center">
                ...
                {formatDocumentType(image.documentType)}
              </span>
            )}
          ...
          {showWorkflowProgress && image.status === 'processing' && workflowStatus && (
            <ProgressBar
              progress={progress}
              colorScheme="blue"
              label={`Processing: ${formatProcessingStep(currentStep)}`}
          ...
          {image.status === 'uploading' && typeof image.progress === 'number' && (
            <ProgressBar
              ...
              secondaryLabel={formatSpeed(image.uploadSpeed || 0)}
            />
          )}

          {/* Enhanced ETA display with proper type checking */}
          {image.status === 'uploading' && typeof extendedImage.eta === 'number' && extendedImage.eta !== Infinity && (
            <p className="text-xs text-gray-500 mt-1">ETA: {formatETA(extendedImage.eta)}</p>
          )}

          {/* Enhanced error handling with type safety */}
          {image.status === 'error' && hasError(image) && (
            <p className="text-sm text-red-600 mt-2">Error: {image.error?.message || 'Unknown error'}</p>
          )}

          {/* Failed steps display for workflow issues */}
          {showWorkflowProgress && workflowStatus && workflowStatus.failedSteps.length > 0 && (
            <p className="text-xs text-red-600 mt-1">
              Failed steps: {workflowStatus.failedSteps.map((s: string) => formatProcessingStep(s)).join(', ')}
            </p>
          )}
          ...
          <div className="mt-3 text-sm text-gray-700 space-y-1">
            {image.metadata?.dimensions && (
              <p>
                Dimensions: {formatDimensions(image.metadata.dimensions.width, image.metadata.dimensions.height)} 
                ({formatAspectRatio(image.metadata.dimensions.width, image.metadata.dimensions.height)})
              </p>
            )}
            {image.metadata?.geoLocation && (
              <p>
                Location: {formatCoordinates(image.metadata.geoLocation.latitude, image.metadata.geoLocation.longitude)} 
                ({formatPropertyLocation(image.metadata.geoLocation.latitude, image.metadata.geoLocation.longitude)})
              </p>
            )}
            ...
            {image.metadata?.createdAt && (
              <p>Uploaded: {formatTimestamp(image.metadata.createdAt, 'short')}</p>
            )}
            ...
            {image.validationResult && (
              <div className="mt-2">
                ...
                {image.validationResult.isValid ? (
                  ...
                  <span className="text-red-600">Failed: {image.validationResult.errors.join(', ')}</span>
                )}
                {image.validationResult.warnings.length > 0 && (
                  <p className="text-orange-600">Warnings: {image.validationResult.warnings.join(', ')}</p>
                )}
              </div>
            )}
              ...
              <div className="mt-2">
                ...
                {image.documentAuthResult?.isAuthentic ? (
                  <span className="text-green-600">
                    Authentic ({formatConfidence(image.documentAuthResult.confidence)})
                  </span>
                ) : (
                  <span className="text-red-600">
                    Not Authentic: {image.documentAuthResult?.anomalies?.join(', ') || 'Unknown issues'}
                  </span>
                )}
              </div>
            )}
            ...
            {fraudRisk && (
              <div className="mt-2">
                ...
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${RISK_LEVEL_COLORS[fraudRisk.level]}`}>
            ...
            {extendedImage.regulatoryFlags && extendedImage.regulatoryFlags.length > 0 && (
              <div className="mt-2">
                <p className="font-medium text-orange-600">Regulatory Flags:</p>
                <ul className="list-disc list-inside text-orange-600">
                  {extendedImage.regulatoryFlags.map((flag: string, i: number) => (
                    <li key={i}>{flag.replace(/_/g, ' ')}</li>
                  ))}
                </ul>
              </div>
          ...
          <div className="mt-4 flex space-x-2">
            ...
            {(image.status === 'uploading' || image.status === 'paused' || image.status === 'error') && hasSessionId(image) && (
              <button 
                type="button"
  ...
  }, [pauseUpload, resumeUpload, cancelUpload, retryUpload, showWorkflowProgress, workflowStats]);
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        {selectedFiles && selectedFiles.length > 0 && (
          <p className="mt-3 text-sm text-gray-600">
            Selected {selectedFiles.length} file(s) for upload as {formatDocumentType(currentDocumentType)}.
          </p>
        )}
      </div>

      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-gray-800">Current Images ({images.length})</h2>
        {images.length > 0 && (
      ...
      <div className="space-y-4">
        {images.length === 0 ? (
          <div className="text-center p-8 bg-white rounded-lg shadow-md text-gray-500">
            ...
            <p className="text-sm mt-2">Start by selecting files above to begin the upload process!</p>
          </div>
        ) : (
