...
const FAQS: FAQ[] = [
  ...
  },
  {
    ...
    answer:
      "We offer a 14-day free trial for our Pro plan, giving you full access to all features with no credit card required.",
  },
  {
    ...
    >
      ...
      {plan.isPopular && (
        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
          <Badge
            className="bg-secondary text-secondary-foreground px-4 py-1 shadow-md"
            role="banner"
          >
            Most Popular
          </Badge>
      ...
      <CardHeader className="text-center pb-8">
        ...
        <div className="mt-4">
          <span
            className={`text-4xl font-bold ${plan.isPopular ? "text-secondary" : "text-primary"}`}
            aria-label={`Price: ${plan.price}${plan.period}`}
          >
            {plan.price}
          </span>
...
const BillingToggle: React.FC<{
  ...
  return (
    <div className="flex items-center justify-center gap-4">
      <span
        className={`text-gray-600 transition-colors ${!isAnnual ? "font-medium text-gray-800" : ""}`}
      >
        Monthly
      </span>
