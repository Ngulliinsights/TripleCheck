 ...
 */
export const useSecureValidation = (schema: ValidationSchema) => {
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [isValidating, setIsValidating] = useState(false);

  const validate = useCallback(async (data: Record<string, any>): Promise<ValidationResult> => {
...
export const useInputSanitization = () => {
  const sanitizeInput = useCallback((input: string, type: 'html' | 'sql' | 'user' = 'user'): string => {
    ...
    switch (type) {
      case 'html':
        return validationService.sanitizeHtml(input);
      case 'sql':
        return validationService.sanitizeSql(input);
      case 'user':
      default:
        return validationService.sanitizeUserInput(input);
    }
  }, []);
