...
import { operationTracker, OperationType } from '@/utils/operation-tracker';
...
export function useTrackedQuery<TData = unknown, TError = Error>(
...
): UseQueryResult<TData, TError> & { operationId?: string } {
    ...
    onSuccess: (data: TData) => {
      if (operationId.current) {
        operationTracker.recordMilestone(
          ...
          data,
          undefined,
          { queryKey, dataSize: JSON.stringify(data).length }
        );
      }
  ...
  const result = useQuery(trackedOptions);
...
export function useTrackedMutation<TData = unknown, TError = Error, TVariables = void>(
  ...
  const result = useMutation(trackedOptions);
