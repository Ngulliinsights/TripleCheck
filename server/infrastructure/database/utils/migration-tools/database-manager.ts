...
class DatabaseManager {
  ...
  private generateRecommendations(report: DatabaseReport): void {
    ...
    if (criticalIssues > 0) {
      ...
      report.recommendations.push('💡 Run: tsx database/scripts/data-pipeline.ts --clear');
    }
    ...
    if (report.stats.users > 0 && report.stats.properties > 0 && report.stats.reviews === 0) {
      ...
      report.recommendations.push('🎯 Run: tsx database/scripts/data-pipeline.ts --no-clear');
    }
