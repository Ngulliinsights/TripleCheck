...
export const fraudAlerts = pgTable(
  ...
  (table) => ({
    alertIdIdx: uniqueIndex("fraud_alerts_alert_id_idx").on(table.alertId),
    userIdx: index("fraud_alerts_user_idx").on(table.userId),
    propertyIdx: index("fraud_alerts_property_idx").on(table.propertyId),
    transactionIdx: index("fraud_alerts_transaction_idx").on(table.transactionId),
    categoryIdx: index("fraud_alerts_category_idx").on(table.category),
    ...
    investigatorIdx: index("fraud_alerts_investigator_idx").on(table.assignedInvestigator),
    activeIdx: index("fraud_alerts_active_idx").on(table.isActive),
    createdAtIdx: index("fraud_alerts_created_at_idx").on(table.createdAt),
    // Composite indexes
    statusSeverityIdx: index("fraud_alerts_status_severity_idx").on(
      table.status,
      table.severity
    ),
    categorySeverityIdx: index("fraud_alerts_category_severity_idx").on(
      table.category,
      table.severity
    ),
  })
...
export const fraudCases = pgTable(
  ...
  (table) => ({
    caseNumberIdx: uniqueIndex("fraud_cases_case_number_idx").on(table.caseNumber),
    categoryIdx: index("fraud_cases_category_idx").on(table.category),
    severityIdx: index("fraud_cases_severity_idx").on(table.severity),
    statusIdx: index("fraud_cases_status_idx").on(table.status),
    priorityIdx: index("fraud_cases_priority_idx").on(table.priority),
    investigatorIdx: index("fraud_cases_investigator_idx").on(table.primaryInvestigator),
    openedAtIdx: index("fraud_cases_opened_at_idx").on(table.openedAt),
    activeIdx: index("fraud_cases_active_idx").on(table.isActive),
    // Composite indexes
    statusPriorityIdx: index("fraud_cases_status_priority_idx").on(
      table.status,
      table.priority
    ),
    categorySeverityIdx: index("fraud_cases_category_severity_idx").on(
      table.category,
      table.severity
    ),
  })
...
export const fraudPatterns = pgTable(
  "fraud_patterns",
  {
    ...
    detectionRules: json("detection_rules").$type<Record<string, any>>().notNull(),
  ...
  (table) => ({
    patternIdIdx: uniqueIndex("fraud_patterns_pattern_id_idx").on(table.patternId),
    categoryIdx: index("fraud_patterns_category_idx").on(table.category),
    typeIdx: index("fraud_patterns_type_idx").on(table.patternType),
    confidenceIdx: index("fraud_patterns_confidence_idx").on(table.confidence),
    activeIdx: index("fraud_patterns_active_idx").on(table.isActive),
    lastTriggeredIdx: index("fraud_patterns_last_triggered_idx").on(table.lastTriggered),
    createdByIdx: index("fraud_patterns_created_by_idx").on(table.createdBy),
  })
);
...
export const complianceReports = pgTable(
  ...
  (table) => ({
    reportIdIdx: uniqueIndex("compliance_reports_report_id_idx").on(table.reportId),
    typeIdx: index("compliance_reports_type_idx").on(table.reportType),
    bodyIdx: index("compliance_reports_body_idx").on(table.regulatoryBody),
    statusIdx: index("compliance_reports_status_idx").on(table.status),
    periodIdx: index("compliance_reports_period_idx").on(table.reportingPeriod),
    submittedByIdx: index("compliance_reports_submitted_by_idx").on(table.submittedBy),
    dueDateIdx: index("compliance_reports_due_date_idx").on(table.dueDate),
    activeIdx: index("compliance_reports_active_idx").on(table.isActive),
    // Composite indexes
    statusDueDateIdx: index("compliance_reports_status_due_date_idx").on(
      table.status,
      table.dueDate
    ),
    bodyPeriodIdx: index("compliance_reports_body_period_idx").on(
