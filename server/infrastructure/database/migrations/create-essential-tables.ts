 ...
 * Creates core tables needed for basic functionality
 */

import { logger } from "../../monitoring/logger";

export async function createEssentialTables(sql: any) {
  try {
    logger.info("Creating essential database tables...", "MIGRATION");
    ...
    logger.info("Creating essential database indexes...", "MIGRATION");
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_properties_created ON properties(created_at)`;
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_reviews_created ON reviews(created_at)`;
    ...
    logger.info("Essential database tables created successfully", "MIGRATION");
    return { success: true };

  } catch (error) {
    logger.error("Failed to create essential database tables", "MIGRATION", { error });
