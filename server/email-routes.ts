...
import { EmailServiceFactory, InquiryClassificationService } from './email-service.js';
import { getEmailConfig, validateEmailConfig } from './email-config.js';
import { EmailService, PropertyInquiry } from '../shared/email-types.js';
...
router.get('/inquiries', async (req, res) => {
  try {
    ...
    const inquiries: PropertyInquiry[] = result.messages.map(message => {
      const classification = InquiryClassificationService.classifyInquiry(message);
      ...
      return {
        ...
        propertyTitle: classification.extractedData.propertyTitle || extractPropertyTitleFromSubject(message.subject),
...
router.post('/inquiries/:id/reply', async (req, res) => {
  try {
    ...
    const messageId = await service.replyToEmail(req.params.id, message, isHtml);
