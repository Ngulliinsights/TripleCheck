...
- ID: ${ref.id}
- Referee ID: ${ref.refereeId}
- Referrer ID: ${ref.referrerId}
- Message: ${ref.message}
- Status: ${ref.status}
`).join('')}
...
- ID: ${end.id}
- Endorsee ID: ${end.endorseeId}
- Endorser ID: ${end.endorserId}
- Type: ${end.type}
- Message: ${end.message}
`).join('')}
...
- ID: ${conn.id}
- Connected User ID: ${conn.connectedUserId}
...
class ComprehensiveTrustCalculator {
  ...
  async calculateComprehensiveTrust(input: ComprehensiveTrustInput): Promise<ComprehensiveTrustResult> {
    ...
    const trustMetrics: TrustMetrics = {
      verificationScore: scoreBreakdown.community,
      communityRating: scoreBreakdown.behavior,
      transactionHistory: scoreBreakdown.transaction,
      responseTime: scoreBreakdown.social,
      profileCompleteness: scoreBreakdown.location
    };
