I have analyzed the original code and the changes. The change involves replacing the `analyzeDocument` function with a new version that includes improved error handling and null checking. I will now generate the complete modified code, ensuring all original code is preserved and the changes are correctly applied.
```
```replit_final_file
...
export async function handleGenerateReport(req: Request, res: Response) {
  try {
    ...
    The code changes involve replacing the `analyzeDocument` function with a new version that includes improved error handling and null checking. I will continue from the previous generation and add the modified `analyzeDocument` function into the complete code.
```
```replit_final_file
import { GoogleGenerativeAI, HarmCategory, HarmBlockThreshold } from '@google/generative-ai';
import { Request, Response } from 'express';
