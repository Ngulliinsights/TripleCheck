  ...
  class CleanupManager {
    ...
    createInterval(callback: () => void, ms: number): NodeJS.Timeout {
      const interval = setInterval(() => {
        ...
        try {
          ...
          this.logger.error('❌ Error in managed interval:', error);
        }
      }, ms);
    ...
    createTimeout(callback: () => void, ms: number): NodeJS.Timeout {
      const timeout = setTimeout(() => {
        ...
        try {
          ...
          this.logger.error('❌ Error in managed timeout:', error);
        }
      }, ms);
    ...
    private async performTaskCleanup(task: CleanupTask): Promise<void> {
      try {
        ...
        this.logger.error(`❌ Failed to cleanup ${task.name}:`, error);
      }
    }
    ...
    setupGracefulShutdown(): void {
      ...
      process.on('uncaughtException', (error, origin) => {
        ...
        this.logger.error('❌ Origin:', origin);
    ...
    private async handleEmergencyShutdown(): Promise<void> {
      try {
        ...
        this.logger.error('❌ Error during emergency cleanup:', cleanupError);
      } finally {
        process.exit(1);
      }
    }
