...
// Import AI functions from existing ai-routes and ai-service
import { detectFraud as aiDetectFraud } from "../routes/ai-routes";
import { 
  detectFraud as aiServiceDetectFraud,
  verifyDocument,
...
export class VerificationService {
  ...
  async generateVerificationReport(propertyId: number): Promise<string> {
    try {
      ...
      return await generateVerificationReport(propertyId);
    } catch (error) {
  ...
  async generateMarketAnalysisReport(propertyId: number): Promise<string> {
    try {
      ...
      return await generateMarketAnalysisReport(property);
    } catch (error) {
  ...
  async generateRiskAssessmentReport(propertyId: number): Promise<string> {
    try {
      ...
      return await generateRiskAssessmentReport(property);
    } catch (error) {
  ...
  private async performAIVerification(property: FraudDetectionInput): Promise<CompleteFraudDetectionResult> {
    ...
    try {
      // Use the enhanced AI service fraud detection
      const result = await aiServiceDetectFraud(property);
