import {
  Shield,
  // Removed unused imports: FileText, Users
  Search,
  CheckCircle,
  ArrowRight,
  Plus,
...
const SERVICE_CATEGORIES: readonly ServiceCategory[] = [
  {
    ...
    contextualInfo: {
      ...
      description:
        "Connect to our verification network and enable comprehensive property checks, fraud detection, and legal compliance validation for Kenya properties.",
      benefits: [
  ...
  },
  {
    ...
    contextualInfo: {
      ...
      description:
        "Reach new opportunities and connect with verified properties, professionals, and market insights tailored to your needs.",
      benefits: [
...
export function ServiceCategories({
  ...
  const handleSubServiceClick = useCallback(
    (subServiceId: string) => {
      setExpandedSubService(
        expandedSubService === subServiceId ? null : subServiceId
      );
    },
    [expandedSubService]
  );
  ...
  const handleServiceAction = useCallback(
    (categoryId: string, subServiceId: string, action: string) => {
  ...
  const getDisplayCategory = (): ServiceCategory | null => {
    if (activeCategory) {
      return (
        SERVICE_CATEGORIES.find((cat) => cat.id === activeCategory) || null
      );
    }
    if (hoveredCategory) {
      return (
        SERVICE_CATEGORIES.find((cat) => cat.id === hoveredCategory) || null
      );
    }
    return null;
  };
  ...
  const getCardStyling = (
    ...
    if (isActive) {
      return `${baseClasses} ${category.bgColor} shadow-2xl shadow-${category.bgColor}/25 scale-105 border-0`;
    }

    if (isHovered) {
      return `${baseClasses} bg-slate-700 border-slate-500 scale-[1.02] shadow-lg`;
    }

    return `${baseClasses} bg-slate-800 hover:bg-slate-700 border border-slate-600 hover:scale-[1.02]`;
  };
  ...
  const getIconStyling = (
    ...
    if (isActive) {
      return `${baseClasses} bg-white/20 text-white scale-110`;
    }

    if (isHovered) {
      return `${baseClasses} ${category.bgColor} text-white scale-105`;
    }
  ...
  const getTitleStyling = (_isActive: boolean, _isHovered: boolean) => {
    const baseClasses =
  ...
  const getStatsStyling = (isActive: boolean, isHovered: boolean) => {
  ...
  return (
    <div
      className={`py-20 bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-900 ${className}`}
    >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {SERVICE_CATEGORIES.map((category) => {
                  ...
                  const isHovered =
                    hoveredCategory === category.id && !activeCategory;
                  const shouldShowPreview = isHovered && !isActive;
                    ...
                    <div key={category.id} className="relative">
                      ...
                      >
                        <CardContent className="p-8 h-full flex flex-col justify-center items-center text-center">
                          ...
                          {category.stats && (
                            <div
                              className={getStatsStyling(isActive, isHovered)}
                            >
                          ...
                          {shouldShowPreview && (
                            <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 text-xs text-white/70 animate-pulse">
                      ...
                      {shouldShowPreview && category.subServices[0] && (
                          ...
                          <div className="bg-slate-700/90 backdrop-blur-sm rounded-lg p-4 border border-slate-500">
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                ...
                                {category.subServices[0].popular && (
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-8">
                {displayCategory ?
                  <Card className="bg-slate-800 border-slate-600 transition-all duration-300">
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-3">
                        {displayCategory.contextualInfo.benefits.map(
                          (benefit, index) => (
                            <div
                              key={index}
