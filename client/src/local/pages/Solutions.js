...
function Solutions() {
    ...
    var _b = (0, react_1.useState)('buyers'), activeTab = _b[0], setActiveTab = _b[1];
    var handleSolutionClick = function (solution, userType) {
        setUserType(userType);
        trackNavigation('/solutions', "/solutions/".concat(solution), 'solution_selection');
        window.location.href = "/solutions/".concat(solution);
    };
    var solutions = {
        buyers: {
    ...
    var currentSolution = solutions[activeTab];
    var IconComponent = currentSolution.icon;
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center mb-16">
            {Object.entries(solutions).map(function (_a) {
            var key = _a[0], solution = _a[1];
            var IconComp = solution.icon;
            return (<button key={key} onClick={function () { return setActiveTab(key); }} className={"flex items-center px-6 py-3 m-2 rounded-lg transition-all duration-300 ".concat(activeTab === key
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-16">
              ...
              <div>
                ...
                <div className="space-y-4 mb-8">
                  {currentSolution.benefits.map(function (benefit, index) { return (<div key={index} className="flex items-center">
                      <lucide_react_1.CheckCircle className="h-5 w-5 text-green-500 mr-3 flex-shrink-0"/>
                      <span className="text-gray-700">{benefit}</span>
                    </div>); })}
                </div>
                
                <button_1.Button size="lg" onClick={function () { return handleSolutionClick(activeTab, activeTab); }} className="flex items-center">
                  {currentSolution.cta}
              ...
              <div className="space-y-6">
                {currentSolution.features.map(function (feature, index) { return (<div key={index} className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-8 text-center">
              <div className="max-w-3xl mx-auto">
                <div className="flex justify-center mb-4">
                  {__spreadArray([], Array(5), true).map(function (_, i) { return (<lucide_react_1.Award key={i} className="h-5 w-5 text-yellow-400 fill-current"/>); })}
                </div>
      ...
      <section className="py-20 bg-gradient-hero-dark text-white">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button_1.Button size="lg" variant="secondary" onClick={function () { return handleSolutionClick(activeTab, activeTab); }} className="flex items-center">
              <lucide_react_1.Clock className="mr-2 h-5 w-5"/>
              Get Started Now
            </button_1.Button>
            <button_1.Button size="lg" variant="outline" className="border-white text-white hover:bg-white hover:text-primary" onClick={function () { return trackNavigation('/solutions', '/contact', 'contact_interest'); }}>
