...
exports.ProfessionalResourcesDirectory = void 0;
...
var PROFESSIONAL_TYPES = {
    surveyor: { label: 'Land Surveyors', icon: <lucide_react_1.Compass className="h-5 w-5"/>, color: 'blue' },
    lawyer: { label: 'Property Lawyers', icon: <lucide_react_1.Scale className="h-5 w-5"/>, color: 'purple' },
    valuer: { label: 'Property Valuers', icon: <lucide_react_1.Award className="h-5 w-5"/>, color: 'green' },
    land_agent: { label: 'Land Agents', icon: <lucide_react_1.Users className="h-5 w-5"/>, color: 'orange' },
    consultant: { label: 'Land Consultants', icon: <lucide_react_1.Users className="h-5 w-5"/>, color: 'gray' }
};
var COUNTIES = [
    'Nairobi', 'Kiambu', 'Nakuru', 'Mombasa', 'Kisumu', 'Eldoret', 'Thika', 'Machakos', 'Meru', 'Nyeri'
];
var ProfessionalResourcesDirectory = function (_a) {
    var _b;
    var onSelectProfessional = _a.onSelectProfessional, filterByType = _a.filterByType, filterByLocation = _a.filterByLocation, _c = _a.showContactInfo, showContactInfo = _c === void 0 ? true : _c;
    var _d = (0, react_1.useState)(''), searchTerm = _d[0], setSearchTerm = _d[1];
    var _e = (0, react_1.useState)(filterByType || 'all'), selectedType = _e[0], setSelectedType = _e[1];
    var _f = (0, react_1.useState)(filterByLocation || 'all'), selectedCounty = _f[0], setSelectedCounty = _f[1];
    var _g = (0, react_1.useState)('all'), selectedPriceRange = _g[0], setSelectedPriceRange = _g[1];
    var _h = (0, react_1.useState)('rating'), sortBy = _h[0], setSortBy = _h[1];
    var _j = (0, react_1.useState)(false), showFilters = _j[0], setShowFilters = _j[1];
    var filteredProfessionals = (0, react_1.useMemo)(function () {
        var filtered = MOCK_PROFESSIONALS.filter(function (professional) {
            var matchesSearch = searchTerm === '' ||
                professional.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                professional.specializations.some(function (spec) { return spec.toLowerCase().includes(searchTerm.toLowerCase()); }) ||
                professional.services.some(function (service) { return service.toLowerCase().includes(searchTerm.toLowerCase()); });
            var matchesType = selectedType === 'all' || professional.type === selectedType;
            var matchesCounty = selectedCounty === 'all' || professional.location.county === selectedCounty;
            var matchesPriceRange = selectedPriceRange === 'all' || professional.priceRange === selectedPriceRange;
    ...
    var renderStars = function (rating) {
        return Array.from({ length: 5 }, function (_, i) { return (<lucide_react_1.Star key={i} className={"h-4 w-4 ".concat(i < Math.floor(rating) ? 'text-yellow-400 fill-current' : 'text-gray-300')}/>); });
    };
    var getPriceRangeColor = function (priceRange) {
        switch (priceRange) {
    ...
    return (<div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input type="text" placeholder="Search professionals, specializations, or services..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
          </div>
          
          <div className="flex gap-2">
            <select value={selectedType} onChange={function (e) { return setSelectedType(e.target.value); }} className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="all">All Types</option>
              {Object.entries(PROFESSIONAL_TYPES).map(function (_a) {
            var key = _a[0], type = _a[1];
            return (<option key={key} value={key}>{type.label}</option>);
        })}
            </select>
            
            <select value={selectedCounty} onChange={function (e) { return setSelectedCounty(e.target.value); }} className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="all">All Counties</option>
              {COUNTIES.map(function (county) { return (<option key={county} value={county}>{county}</option>); })}
            </select>
            
            <button onClick={function () { return setShowFilters(!showFilters); }} className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center gap-2">
        ...
        {showFilters && (<div className="mt-4 pt-4 border-t border-gray-200">
            <div className="grid md:grid-cols-3 gap-4">
              <div>
                ...
                <select value={selectedPriceRange} onChange={function (e) { return setSelectedPriceRange(e.target.value); }} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <select value={sortBy} onChange={function (e) { return setSortBy(e.target.value); }} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <div className="mb-4">
        <p className="text-gray-600">
          ...
          {selectedType !== 'all' && " in ".concat((_b = PROFESSIONAL_TYPES[selectedType]) === null || _b === void 0 ? void 0 : _b.label)}
          {selectedCounty !== 'all' && " in ".concat(selectedCounty, " County")}
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {filteredProfessionals.map(function (professional) {
            var typeInfo = PROFESSIONAL_TYPES[professional.type];
            return (<div key={professional.id} className="bg-white rounded-lg shadow-sm border hover:shadow-md transition-shadow">
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-3">
                    <div className={"p-2 rounded-lg bg-".concat(typeInfo.color, "-100 text-").concat(typeInfo.color, "-600")}>
                      {typeInfo.icon}
                    </div>
                    <div>
                  ...
                  <div className="flex flex-col items-end space-y-2">
                    <span className={"px-2 py-1 rounded text-xs font-medium ".concat(getAvailabilityColor(professional.availability))}>
                      {professional.availability.charAt(0).toUpperCase() + professional.availability.slice(1)}
                    </span>
                    <span className={"px-2 py-1 rounded text-xs font-medium ".concat(getPriceRangeColor(professional.priceRange))}>
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {professional.specializations.slice(0, 3).map(function (spec) { return (<span key={spec} className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded">
                          {spec}
                        </span>); })}
                      {professional.specializations.length > 3 && (<span className="text-xs text-gray-500">
                ...
                {showContactInfo && (<div className="mb-4">
                    ...
                    <div className="space-y-1">
                      ...
                      {professional.contact.website && (<div className="flex items-center text-sm text-blue-600">
                          <lucide_react_1.ExternalLink className="h-4 w-4 mr-2"/>
                          <a href={"https://".concat(professional.contact.website)} target="_blank" rel="noopener noreferrer" className="hover:underline">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-200">
                  ...
                  {onSelectProfessional && (<button onClick={function () { return onSelectProfessional(professional); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">
                      Select Professional
                    </button>)}
                </div>
              </div>
      ...
      {filteredProfessionals.length === 0 && (<div className="text-center py-12">
...
exports.ProfessionalResourcesDirectory = ProfessionalResourcesDirectory;
exports.default = exports.ProfessionalResourcesDirectory;
