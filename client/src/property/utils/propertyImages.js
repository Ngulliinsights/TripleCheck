...
exports.ALL_PROPERTY_IMAGES = exports.RESIDENTIAL_IMAGES = exports.COMMERCIAL_IMAGES = void 0;
exports.getImagesByPropertyType = getImagesByPropertyType;
exports.getRandomImages = getRandomImages;
exports.getRandomImage = getRandomImage;
exports.imagesToUrls = imagesToUrls;
// Commercial property images
exports.COMMERCIAL_IMAGES = [
    {
        id: 'comm-1',
...
exports.ALL_PROPERTY_IMAGES = __spreadArray(__spreadArray([], exports.COMMERCIAL_IMAGES, true), exports.RESIDENTIAL_IMAGES, true);
/**
 * Get images based on property type
 */
function getImagesByPropertyType(propertyType) {
    var isCommercial = propertyType === 'commercial';
    if (isCommercial) {
        return exports.COMMERCIAL_IMAGES;
    }
    // For residential types (apartment, house, villa, townhouse, land)
    return exports.RESIDENTIAL_IMAGES;
}
/**
 * Get random images for a property type
 */
function getRandomImages(propertyType, count) {
    if (count === void 0) { count = 3; }
    var availableImages = getImagesByPropertyType(propertyType);
    var shuffled = __spreadArray([], availableImages, true).sort(function () { return 0.5 - Math.random(); });
    return shuffled.slice(0, count);
}
/**
 * Get a single random image for a property type
 */
function getRandomImage(propertyType) {
    var images = getImagesByPropertyType(propertyType);
    var randomImage = images[Math.floor(Math.random() * images.length)];
    return randomImage || images[0] || { id: 'default', url: '/images/default-property.jpg', alt: 'Default property image', category: 'residential' };
}
/**
 * Convert PropertyImage array to URL strings for API
 */
function imagesToUrls(images) {
    return images.map(function (img) { return img.url; });
}
