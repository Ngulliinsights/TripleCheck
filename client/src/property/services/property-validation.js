"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PropertyBusinessLogic = exports.PropertySearchSchema = exports.PropertySchema = void 0;
var zod_1 = require("zod");
// Property validation schemas
exports.PropertySchema = zod_1.z.object({
    title: zod_1.z.string()
        .min(10, 'Title must be at least 10 characters')
        .max(100, 'Title must not exceed 100 characters')
        .regex(/^[a-zA-Z0-9\s\-,.']+$/, 'Title contains invalid characters'),
    description: zod_1.z.string()
        .min(50, 'Description must be at least 50 characters')
        .max(2000, 'Description must not exceed 2000 characters'),
    price: zod_1.z.number()
        .positive('Price must be positive')
        .min(1000, 'Minimum price is $1,000')
        .max(50000000, 'Maximum price is $50,000,000'),
    location: zod_1.z.object({
        address: zod_1.z.string().min(10, 'Address must be at least 10 characters'),
        city: zod_1.z.string().min(2, 'City must be at least 2 characters'),
        state: zod_1.z.string().min(2, 'State must be at least 2 characters'),
        country: zod_1.z.string().min(2, 'Country must be at least 2 characters'),
        coordinates: zod_1.z.object({
            lat: zod_1.z.number().min(-90).max(90),
            lng: zod_1.z.number().min(-180).max(180),
        }).optional(),
    }),
    propertyType: zod_1.z.enum(['apartment', 'house', 'condo', 'townhouse', 'land']),
    bedrooms: zod_1.z.number()
        .int('Bedrooms must be a whole number')
        .min(0, 'Bedrooms cannot be negative')
        .max(20, 'Maximum 20 bedrooms allowed'),
    bathrooms: zod_1.z.number()
        .min(0, 'Bathrooms cannot be negative')
        .max(20, 'Maximum 20 bathrooms allowed'),
    area: zod_1.z.number()
        .positive('Area must be positive')
        .min(100, 'Minimum area is 100 sq ft')
        .max(100000, 'Maximum area is 100,000 sq ft'),
    images: zod_1.z.array(zod_1.z.string().url('Invalid image URL'))
        .min(1, 'At least one image is required')
        .max(20, 'Maximum 20 images allowed'),
    amenities: zod_1.z.array(zod_1.z.string())
        .max(50, 'Maximum 50 amenities allowed'),
    ownerId: zod_1.z.string().uuid('Invalid owner ID'),
    status: zod_1.z.enum(['active', 'pending', 'sold', 'inactive']).default('pending'),
    verificationStatus: zod_1.z.enum(['pending', 'verified', 'rejected']).default('pending'),
});
exports.PropertySearchSchema = zod_1.z.object({
    query: zod_1.z.string().default(''),
    location: zod_1.z.string().optional(),
    priceMin: zod_1.z.number().positive().optional(),
    priceMax: zod_1.z.number().positive().optional(),
    propertyType: zod_1.z.enum(['apartment', 'house', 'condo', 'townhouse', 'land']).optional(),
    bedrooms: zod_1.z.number().int().min(0).optional(),
    bathrooms: zod_1.z.number().min(0).optional(),
    areaMin: zod_1.z.number().positive().optional(),
    areaMax: zod_1.z.number().positive().optional(),
    amenities: zod_1.z.array(zod_1.z.string()).optional(),
    landVerified: zod_1.z.boolean().optional(),
    landRiskLevel: zod_1.z.enum(['low', 'medium', 'high', 'critical']).optional(),
    page: zod_1.z.number().int().positive().default(1),
    limit: zod_1.z.number().int().positive().max(100).default(12),
    sortBy: zod_1.z.enum(['price', 'date', 'relevance', 'trustScore', 'landVerification']).default('relevance'),
    sortOrder: zod_1.z.enum(['asc', 'desc']).default('desc'),
});
// Business logic for property operations
var PropertyBusinessLogic = /** @class */ (function () {
    function PropertyBusinessLogic() {
    }
    // Validate property data
    PropertyBusinessLogic.validateProperty = function (data) {
        try {
            return exports.PropertySchema.parse(data);
        }
        catch (error) {
            if (error instanceof zod_1.z.ZodError) {
                var errorMessages = error.errors.map(function (err) { return "".concat(err.path.join('.'), ": ").concat(err.message); });
                throw new Error("Property validation failed: ".concat(errorMessages.join(', ')));
            }
            throw error;
        }
    };
    // Validate search parameters
    PropertyBusinessLogic.validateSearchParams = function (params) {
        try {
            var parsed = exports.PropertySearchSchema.parse(params);
            // Type assertion to ensure compatibility with PropertySearchParams
            return parsed;
        }
        catch (error) {
            if (error instanceof zod_1.z.ZodError) {
                var errorMessages = error.errors.map(function (err) { return "".concat(err.path.join('.'), ": ").concat(err.message); });
                throw new Error("Search validation failed: ".concat(errorMessages.join(', ')));
            }
            throw error;
        }
    };
    // Calculate property score based on various factors
    PropertyBusinessLogic.calculatePropertyScore = function (property) {
        var score = 0;
        // Base score from property type
        var typeScores = {
            house: 100,
            condo: 90,
            townhouse: 85,
            apartment: 80,
            land: 70,
        };
        if (property.propertyType) {
            score += typeScores[property.propertyType] || 0;
        }
        // Image quality score (more images = higher score)
        if (property.images) {
            score += Math.min(property.images.length * 5, 50);
        }
        // Description quality score
        if (property.description && property.description.length > 200)
            score += 20;
        if (property.description && property.description.length > 500)
            score += 10;
        // Amenities score
        if (property.amenities) {
            score += Math.min(property.amenities.length * 2, 30);
        }
        // Location score (if coordinates provided)
        if (typeof property.location === 'object' && property.location.coordinates)
            score += 15;
        // Verification bonus
        if (property.verificationStatus === 'verified')
            score += 50;
        // Land verification integration
        if (property.landVerification) {
            score += this.calculateLandVerificationScore(property.landVerification);
        }
        // Trust score integration
        if (property.trustScore) {
            score += Math.floor(property.trustScore / 10);
        }
        return Math.min(score, 1000); // Cap at 1000
    };
    // Calculate land verification contribution to property score
    PropertyBusinessLogic.calculateLandVerificationScore = function (landVerification) {
        if (!landVerification)
            return 0;
        var score = 0;
        // Base score for having land verification
        score += 20;
        // Status-based scoring
        switch (landVerification.status) {
            case 'completed':
                score += 60;
                break;
            case 'in_progress':
                score += 30;
                break;
            case 'suspended':
                score += 10;
                break;
            case 'failed':
                score -= 20;
                break;
            default:
                break;
        }
        // Risk level adjustment
        switch (landVerification.riskLevel) {
            case 'low':
                score += 40;
                break;
            case 'medium':
                score += 20;
                break;
            case 'high':
                score -= 10;
                break;
            case 'critical':
                score -= 30;
                break;
        }
        // Confidence bonus
        score += Math.floor(landVerification.confidence * 20);
        // Completed layers bonus
        score += landVerification.completedLayers.length * 5;
        return Math.max(0, score);
    };
    // Generate land verification badge based on status
    PropertyBusinessLogic.generateLandVerificationBadge = function (landVerification) {
        if (!landVerification)
            return undefined;
        switch (landVerification.status) {
            case 'completed':
                if (landVerification.riskLevel === 'low') {
                    return {
                        type: 'verified',
                        label: 'Land Verified',
                        color: 'green',
                        description: 'Property has completed comprehensive land verification with low risk'
                    };
                }
                else if (landVerification.riskLevel === 'medium') {
                    return {
                        type: 'verified',
                        label: 'Land Verified - Medium Risk',
                        color: 'blue',
                        description: 'Property has completed land verification with medium risk factors identified'
                    };
                }
                else {
                    return {
                        type: 'high_risk',
                        label: 'High Risk Property',
                        color: 'red',
                        description: 'Property has completed verification but significant risks were identified'
                    };
                }
            case 'in_progress':
                return {
                    type: 'in_progress',
                    label: 'Verification In Progress',
                    color: 'blue',
                    description: 'Land verification is currently underway'
                };
            case 'suspended':
            case 'failed':
                return {
                    type: 'expert_required',
                    label: 'Expert Review Required',
                    color: 'orange',
                    description: 'Land verification requires expert attention'
                };
            default:
                return undefined;
        }
    };
    // Determine if property is featured based on score and other factors
    PropertyBusinessLogic.isFeaturedProperty = function (property) {
        var _a, _b, _c;
        var score = this.calculatePropertyScore(property);
        var hasBasicRequirements = property.verificationStatus === 'verified' &&
            property.isActive === true &&
            (property.images || property.imageUrls || []).length >= 5;
        // Enhanced requirements for land properties
        if (((_a = property.features) === null || _a === void 0 ? void 0 : _a.propertyType) === 'land') {
            return score >= 850 &&
                hasBasicRequirements &&
                ((_b = property.landVerification) === null || _b === void 0 ? void 0 : _b.status) === 'completed' &&
                ((_c = property.landVerification) === null || _c === void 0 ? void 0 : _c.riskLevel) === 'low';
        }
        // Standard requirements for other property types
        return score >= 800 && hasBasicRequirements;
    };
    // Calculate estimated market value based on similar properties
    PropertyBusinessLogic.estimateMarketValue = function (property, similarProperties) {
        var _a;
        if (similarProperties.length === 0) {
            return {
                estimatedValue: typeof property.price === 'string' ? parseInt(property.price, 10) : property.price,
                confidence: 0,
                factors: ['No similar properties found for comparison'],
            };
        }
        // Calculate price per square foot for similar properties
        var pricePerSqFt = similarProperties
            .filter(function (p) { var _a; return (p.area || ((_a = p.features) === null || _a === void 0 ? void 0 : _a.squareFeet) || 0) > 0; })
            .map(function (p) {
            var _a;
            var price = typeof p.price === 'string' ? parseInt(p.price, 10) : p.price;
            var area = p.area || ((_a = p.features) === null || _a === void 0 ? void 0 : _a.squareFeet) || 1;
            return price / area;
        });
        if (pricePerSqFt.length === 0) {
            return {
                estimatedValue: typeof property.price === 'string' ? parseInt(property.price, 10) : property.price,
                confidence: 0,
                factors: ['Insufficient area data for comparison'],
            };
        }
        var avgPricePerSqFt = pricePerSqFt.reduce(function (sum, price) { return sum + price; }, 0) / pricePerSqFt.length;
        var propertyArea = property.area || ((_a = property.features) === null || _a === void 0 ? void 0 : _a.squareFeet) || 1;
        var estimatedValue = Math.round(avgPricePerSqFt * propertyArea);
        // Calculate confidence based on number of similar properties and variance
        var variance = pricePerSqFt.reduce(function (sum, price) { return sum + Math.pow(price - avgPricePerSqFt, 2); }, 0) / pricePerSqFt.length;
        var standardDeviation = Math.sqrt(variance);
        var coefficientOfVariation = standardDeviation / avgPricePerSqFt;
        var confidence = Math.max(0, 100 - (coefficientOfVariation * 100));
        confidence = Math.min(confidence, similarProperties.length * 10); // More properties = higher confidence
        var factors = [
            "Based on ".concat(similarProperties.length, " similar properties"),
            "Average price per sq ft: $".concat(avgPricePerSqFt.toFixed(2)),
            "Property area: ".concat(propertyArea, " sq ft"),
        ];
        if (coefficientOfVariation > 0.3) {
            factors.push('High price variance in similar properties');
        }
        return {
            estimatedValue: Math.round(estimatedValue),
            confidence: Math.round(confidence),
            factors: factors,
        };
    };
    // Generate property recommendations based on user preferences
    PropertyBusinessLogic.generateRecommendations = function (userPreferences, availableProperties) {
        var _this = this;
        return availableProperties
            .filter(function (property) {
            var _a, _b;
            // Price filter
            var price = typeof property.price === 'string' ? parseInt(property.price, 10) : property.price;
            if (price < userPreferences.priceRange.min ||
                price > userPreferences.priceRange.max) {
                return false;
            }
            // Type filter
            if (userPreferences.preferredTypes.length > 0 &&
                !userPreferences.preferredTypes.includes(((_a = property.features) === null || _a === void 0 ? void 0 : _a.propertyType) || '')) {
                return false;
            }
            // Bedroom filter
            if ((((_b = property.features) === null || _b === void 0 ? void 0 : _b.bedrooms) || 0) < userPreferences.minBedrooms) {
                return false;
            }
            // Status filter
            return property.isActive === true;
        })
            .map(function (property) { return (__assign(__assign({}, property), { matchScore: _this.calculateMatchScore(property, userPreferences) })); })
            .sort(function (a, b) { return b.matchScore - a.matchScore; })
            .slice(0, 10); // Top 10 recommendations
    };
    PropertyBusinessLogic.calculateMatchScore = function (property, preferences) {
        var _a, _b;
        var score = 0;
        // Amenity matching
        var preferredAmenities = Array.isArray(preferences.preferredAmenities) ? preferences.preferredAmenities : [];
        var matchingAmenities = (((_a = property.features) === null || _a === void 0 ? void 0 : _a.amenities) || []).filter(function (amenity) {
            return preferredAmenities.includes(amenity);
        });
        score += matchingAmenities.length * 10;
        // Location matching (simple string matching for now)
        var preferredLocation = typeof preferences.location === 'string' ? preferences.location : '';
        if (preferredLocation &&
            (typeof property.location === 'string' ? property.location : ((_b = property.location) === null || _b === void 0 ? void 0 : _b.city) || '').toLowerCase().includes(preferredLocation.toLowerCase())) {
            score += 20;
        }
        // Property score bonus
        score += this.calculatePropertyScore(property) / 10;
        // Verification bonus
        if (property.verificationStatus === 'verified') {
            score += 15;
        }
        return score;
    };
    // Validate property ownership
    PropertyBusinessLogic.validateOwnership = function (property, userId) {
        return property.ownerId === userId;
    };
    // Check if property can be edited
    PropertyBusinessLogic.canEditProperty = function (property, userId) {
        var reasons = [];
        var canEdit = true;
        // Ownership check
        if (!this.validateOwnership(property, userId)) {
            canEdit = false;
            reasons.push('You are not the owner of this property');
        }
        // Status check
        if (property.verificationStatus === 'unverified') {
            canEdit = false;
            reasons.push('Unverified properties have limited editing capabilities');
        }
        // Verification check
        if (property.verificationStatus === 'verified' && property.isActive === true) {
            reasons.push('Verified active properties have limited editing options');
        }
        return { canEdit: canEdit, reasons: reasons };
    };
    // Generate property listing URL
    PropertyBusinessLogic.generateListingUrl = function (property) {
        var slug = property.title
            .toLowerCase()
            .replace(/[^a-z0-9\s-]/g, '')
            .replace(/\s+/g, '-')
            .replace(/-+/g, '-')
            .trim();
        return "/property/".concat(property.id, "/").concat(slug);
    };
    return PropertyBusinessLogic;
}());
exports.PropertyBusinessLogic = PropertyBusinessLogic;
